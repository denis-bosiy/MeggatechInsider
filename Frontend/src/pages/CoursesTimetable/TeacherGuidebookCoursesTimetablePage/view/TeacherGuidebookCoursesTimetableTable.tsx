import { AvailableTime, TeacherGuidebookCoursesTimetableData } from "../model/types";
...
const TeacherGuidebookCoursesTimetableTable = ({
  ...
  const rows = data.map((item) => (
    <tr className="row" key={item.id}>
      <td className="cell">{item.course}</td>
      <td className="cell">{item.type}</td>
      <td className="cell">{item.teacherName}</td>
      <td className="cell">
        {isEdited ? (
          <Multiselect
            defaultValue={availableTimes.filter((time) => item.availableTimes.includes(time.id)).map(getSelectItem)}
  ...
  return (
    <>
      <thead className="header">
        <tr className="row">
          <th className="cell -filter" onClick={() => handleSort("course")}>
            Курс
          </th>
          ...
          <th className="cell -filter" onClick={() => handleSort("availableHours")}>
            Доступные часы
          </th>
