...
const Table = ({ data, handleSort }: TableProps) => {
  ...
  return (
    <>
      <thead className="header">
        <tr className="row">
          <th className="cell -filter" onClick={() => handleSort("name")} rowSpan={2}>
            Курс
          </th>
          <th className="cell -filter" onClick={() => handleSort("type")} rowSpan={2}>
            Тип
          </th>
          {!!data.length &&
            data[0].groups.map((group) => (
              <th key={group.id} className="cell" colSpan={3}>
                {group.name}
              </th>
            ))}
        </tr>
  ...
  return (
    <>
      ...
      <table className="table">
        <Table data={state.data} handleSort={actions.handleSort} />
      </table>
    </>
  );
};
