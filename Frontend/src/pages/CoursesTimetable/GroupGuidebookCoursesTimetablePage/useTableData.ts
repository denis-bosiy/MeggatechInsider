...
function useTableData() {
  const data = useSelector(
    (state: { groupGuidebookCoursesTimetablePageStore: GroupGuidebookCoursesTimetablePageData }) =>
  ...
  const groupGuidebookCoursesTimetableTableBuilder: CTableBuilder = new CTableBuilder(tableData, setTableData);
  ...
  const groupGuidebookCoursesTimetableTableManager: CTableManager = new CTableManager(
    groupGuidebookCoursesTimetableTable
  );

  const handleSort = (columnName: string) => {
    groupGuidebookCoursesTimetableTableManager.invokeFunction("sort", TableType.Default, [
      columnName,
  ...
  const handleSearch = (searchValue: string) => {
    groupGuidebookCoursesTimetableTableManager.invokeFunction("search", TableType.Searchable, [searchValue, data]);
  };
