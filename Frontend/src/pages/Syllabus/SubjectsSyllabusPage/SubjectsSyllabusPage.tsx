      ...
      <table className="table -fill -list">
        <thead className="header">
          <tr className="row">
            ...
            <th className="cell -filter" onClick={() => handleSort("surchargeForNotebooks")}>Доплата за<br />тетради, %</th>
            <th className="cell -filter" onClick={() => handleSort("numberOf10")}>Планируемое<br />кол-во<br />часов 10 кл.</th>
            <th className="cell -filter" onClick={() => handleSort("numberOfGroupsIn10")}>Число<br />групп<br />10 кл.</th>
            <th className="cell -filter" onClick={() => handleSort("numberOf11")}>Планируемое<br />кол-во<br />часов 11 кл.</th>
            <th className="cell -filter" onClick={() => handleSort("numberOfGroupsIn11")}>Число<br />групп<br />11 кл.</th>
            <th className="cell -filter" onClick={() => handleSort("isFinalExam")}>ЕГЭ</th>
          </tr>
        </thead>
        <tbody>
          {subjectsTableData.filter((data: SubjectSyllabusData, index: number) =>
              ...
              <Select
                ...
                onValueChange={(newValue: string) => {
                  ...
                  if (selectedOption) {
                    setSubjectsTableData(
                      subjectsTableData.map((data: SubjectSyllabusData) =>
                        data.id === subjectsTableData[subjectsTableData.length - 1].id ? {...data, financing: selectedOption.content} : data
                      )
                    );
                  }
              ...
              <Select
                ...
                onValueChange={(newValue: string) => {
                  ...
                  if (selectedOption) {
                    setSubjectsTableData(
                      subjectsTableData.map((data: SubjectSyllabusData) =>
                        data.id === subjectsTableData[subjectsTableData.length - 1].id ? {...data, category: selectedOption.content} : data
                      )
                    );
                  }
                }}
            ...
            <td className="cell">
              <CheckBox
                onChange={(event) => {
                  setSubjectsTableData(
                    subjectsTableData.map((data: SubjectSyllabusData) =>
                      data.id === subjectsTableData[subjectsTableData.length - 1].id ? { ...data, isFinalExam: event } : data
                    )
                  );
                }}
              />
