      ...
      <table className="table -fill -list">
        <thead className="header">
          <tr className="row">
            ...
            <th className="cell -filter" onClick={() => handleSort("category")}>Категория</th>
            <th className="cell -filter" onClick={() => handleSort("categoryPayrollAccounting")}>Учет в зп</th>
            <th className="cell -filter" onClick={() => handleSort("workingContract")}>Договор</th>
            <th className="cell -filter" onClick={() => handleSort("workingContractPayrollAccounting")}>Учет в зп</th>
            <th className="cell -filter" onClick={() => handleSort("education")}>Образование</th>
            <th className="cell -filter" onClick={() => handleSort("isClassroomTeacher")}>Класс.<br />рук-во</th>
            <th className="cell -filter" onClick={() => handleSort("inDepthSubjectPayrollAccounting")}>Учет углуб. в зп</th>
            <th className="cell -filter" onClick={() => handleSort("finalExamPayrollAccounting")}>Учет ЕГЭ в зп</th>
            <th className="cell -filter" onClick={() => handleSort("workingStartDate")}>Дата<br />начала<br />работы</th>
            ...
            <th className="cell -filter" onClick={() => handleSort("birthDay")}>Дата<br />рождения</th>
            <th className="cell -filter" onClick={() => handleSort("age")}>Возраст, лет</th>
          </tr>
        </thead>
        <tbody>
          {teachersTableData.filter((data: TeacherSyllabusData, index: number) =>
              ...
              <Select
                ...
                onValueChange={(newValue: string) => {
                  ...
                  if (selectedOption) {
                    setTeachersTableData(
                      teachersTableData.map((data: TeacherSyllabusData) =>
                        data.id === teachersTableData[teachersTableData.length - 1].id ? {...data, category: selectedOption.content} : data
                      )
                    );
                  }
                }}
            ...
            <td className="cell">
              <CheckBox
                onChange={(event) => {
                  setTeachersTableData(
                    teachersTableData.map((data: TeacherSyllabusData) =>
                      data.id === teachersTableData[teachersTableData.length - 1].id ? { ...data, categoryPayrollAccounting: event } : data
                    )
                  );
                }}
              />
            ...
            <td className="cell">
              <CheckBox
                onChange={(event) => {
                  setTeachersTableData(
                    teachersTableData.map((data: TeacherSyllabusData) =>
                      data.id === teachersTableData[teachersTableData.length - 1].id ? { ...data, workingContractPayrollAccounting: event } : data
                    )
                  );
                }}
              />
              ...
              <Select
                ...
                onValueChange={(newValue: string) => {
                  ...
                  if (selectedOption) {
                    setTeachersTableData(
                      teachersTableData.map((data: TeacherSyllabusData) =>
                        data.id === teachersTableData[teachersTableData.length - 1].id ? {...data, education: selectedOption.content} : data
                      )
                    );
                  }
                }}
            ...
            <td className="cell">
              <CheckBox
                onChange={(event) => {
                  setTeachersTableData(
                    teachersTableData.map((data: TeacherSyllabusData) =>
                      data.id === teachersTableData[teachersTableData.length - 1].id ? { ...data, isClassroomTeacher: event } : data
                    )
                  );
                }}
              />
            ...
            <td className="cell">
              <CheckBox
                onChange={(event) => {
                  setTeachersTableData(
                    teachersTableData.map((data: TeacherSyllabusData) =>
                      data.id === teachersTableData[teachersTableData.length - 1].id ? { ...data, inDepthSubjectPayrollAccounting: event } : data
                    )
                  );
                }}
              />
            ...
            <td className="cell">
              <CheckBox
                onChange={(event) => {
                  setTeachersTableData(
                    teachersTableData.map((data: TeacherSyllabusData) =>
                      data.id === teachersTableData[teachersTableData.length - 1].id ? { ...data, finalExamPayrollAccounting: event } : data
                    )
                  );
                }}
              />
